...
export default class FomoMiner {
    constructor(params = {}) {
        ...
        this.minPrice = "1000000000000000";
        this.totalBalanceFomo = 0;
        this.coinObjectCount = 0;
    ...
    async fetchBusWithId(randomBusId) {
        const bus = new (this._suiMaster.SuiObject)({ id: randomBusId, suiMaster: this._suiMaster });
        await bus.fetchFields();
        return bus;
    }
    ...
    async fetchBus2() {
        await this.checkObjects();
        const buses = await Promise.all(this._buses.map(busId => this.fetchBusWithId(busId)));
        const maxRewardBus = buses.reduce((max, bus) => {
            return parseFloat(bus.fields.reward_rate) > parseFloat(max.fields.reward_rate) ? bus : max;
        });
    ...
    async mine(startNonce = 0) {
        ...
        if (parseFloat(bus.fields.reward_rate || 0) < parseFloat(this.minPrice)) {
            console.log("不够成本了");
        ...
        let rewards = parseFloat(bus.fields.rewards);
        let reward_rate = parseFloat(bus.fields.reward_rate);
        let epoch_hashes = parseFloat(bus.fields.epoch_hashes);

        try {
            if (rewards <= 0) return true;
            let totalReward = reward_rate * epoch_hashes + rewards;
            if (reward_rate * epoch_hashes / totalReward > 0.82) {
                console.log("奖金池不够");
                return true;
            }
        } catch (error) {
        ...
        // console.log(preparedHash,difficultyAsTarget)
        // return
    ...
    async submit(nonce, bus, miner) {
        ...
        try {
            let bus = await this.fetchBus();
            // console.log(bus.fields.rewards, bus.fields.reward_rate)
            // console.log(bus.fields)
            ...
            if (parseFloat(bus.fields.reward_rate || 0) > parseFloat(bus.fields.rewards || 0)) {
                console.log("奖金池不够");
                return true;
            }
